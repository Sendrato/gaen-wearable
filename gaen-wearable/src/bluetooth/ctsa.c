...
void bt_gatt_ctsa_buf2timespec(uint8_t *buf, struct timespec* ts)
{
    ...
    LOG_DBG(" >> data: %d / %d / %d - %d:%d:%d", now_tm.tm_year, now_tm.tm_mon, now_tm.tm_mday, now_tm.tm_hour, now_tm.tm_min, now_tm.tm_sec);
...
void bt_gatt_ctsa_now2buf(uint8_t *buf)
{
    ...
    LOG_DBG(" >> data: %d / %d / %d - %d:%d:%d", now_tm.tm_year, now_tm.tm_mon,
